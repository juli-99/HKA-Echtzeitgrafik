...
GeometryBuffer::GeometryBuffer(std::vector<float>& vertices, std::vector<unsigned int>& indices)
    : vao_(0), vbo_(0), ebo_(0), hasEBO_(false)
{
    glGenVertexArrays(1, &vao_);
    glGenBuffers(1, &vbo_); //1 for 1 Buffer for the Object, Reference

    uploadVertexData(vertices.data(), vertices.size() * sizeof(float));
    uploadIndexData(indices.data(), indices.size() * sizeof(float));  
}
