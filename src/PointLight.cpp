...
const std::map<int, const std::array<const float, 3>> PointLight::attenuation = {
	{7,   {1.0, 0.7,   1.8}},
	{13,  {1.0, 0.35,  0.44}},
	{20,  {1.0, 0.22,  0.20}},
	{32,  {1.0, 0.14,  0.07}},
...
void PointLight::applyToShader(Shader shader, int distance)
{
	...
	shader.setUniform(colorLoc, this->color);
	int constLoc = shader.getUniformLoc("u_Light.constant");
	shader.setUniform(constLoc, attenuation.at(distance)[0]);
	int linLoc = shader.getUniformLoc("u_Light.lin");
	shader.setUniform(linLoc, attenuation.at(distance)[1]);
	int quadLoc = shader.getUniformLoc("u_Light.quad");
	shader.setUniform(quadLoc, attenuation.at(distance)[2]);
}